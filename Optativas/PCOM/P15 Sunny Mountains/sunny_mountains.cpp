...
void resuelveCaso() {
    ...
    int size = int(mountains.size());
    T height = 0;
    double result = 0.0;
    // double result = dist(mountains[size - 1], mountains[size - 2]);
    for (int i = size - 1; i > 0; i--) {
        pt p = mountains[i], next = mountains[i - 1];
        if (next.y > height) {
            // obtener la interseccion de la recta {p.x, height}->{0, height} en la recta p->next
            line current(p, next), aux({p.x, height}, {0.0, height});
            pt projection;
            if (inter(current, aux, projection)) {
                // actualizar el resulta con result += dist(proj, next)
                result += dist(projection, next);
            }
        }
        if (lt(height, next.y)) {
            height = next.y;
        }
        // height = max(height, next.y);
    }
