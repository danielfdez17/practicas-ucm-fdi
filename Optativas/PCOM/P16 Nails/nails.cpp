...
void resuelveCaso() {
    double initial; int nails; cin >> initial >> nails;
    vector<pt> vNails, convex_hull;
    for (int i = 0; i < nails; i++) {
        T x, y; cin >> x >> y;
        vNails.push_back({x, y});
    }
    convex_hull = convexHull(vNails);
    double size = 0.0;
    for (int i = 0; i < int(convex_hull.size()) - 1; i++) {
        pt current = convex_hull[i], next = convex_hull[i + 1];
        size += dist(current, next);
    }
    if (lt(initial, size)) initial = size;
    cout << fixed << setprecision(5) << initial << "\n";
}
