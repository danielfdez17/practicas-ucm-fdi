...
public class GUIBuscarCurso extends JFrame implements GUI {
	...
	private static final String FROM_WHERE = "GUIBuscarCurso";
	private JTextField textId;
	private static GUIBuscarCurso instancia;
	
	public GUIBuscarCurso() {
		this.initGUI();
	}

	public static synchronized GUIBuscarCurso getInstancia() {
		if (instancia == null) instancia = new GUIBuscarCurso();
		return instancia;
	}
	
	private void initGUI() {
		this.setTitle("Buscar curso");
		...
		buttonBuscar.addActionListener((e) -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					this.clear();
					Controlador.getInstancia().accion(new Context(Eventos.BUSCAR_CURSO, id));
	...
	public void update(Context context) {
		...
		case BUSCAR_CURSO_KO:
			GUIMSG.showMessage("El curso no existe", FROM_WHERE, true);
		default:
