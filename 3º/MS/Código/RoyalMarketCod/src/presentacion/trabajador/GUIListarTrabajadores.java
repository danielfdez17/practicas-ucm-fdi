...
public class GUIListarTrabajadores extends JFrame implements GUI {
	private static final String FROM_WHERE = "GUIListarTrabajadores";
	private static GUIListarTrabajadores instancia;
	private String headers[] = { "ID ", "Telefono", "Nif", "Nombre", "Direccion", "ID almacen", "Sueldo base", "Horas", "Precio hora"};
	private JTable table;
	private DefaultTableModel model;
	
	public GUIListarTrabajadores() {
		this.initGUI();
	}
	
	public synchronized static GUIListarTrabajadores getInstancia() {
		if (instancia == null) instancia = new GUIListarTrabajadores();
		return instancia;
	}
    ...
    private void initGUI() {
    	...
    	button_listar.addActionListener((e) -> {
    		setVisible(false);
    		Controlador.getInstancia().accion(new Context(Eventos.LISTAR_TRABAJADORES, null));
	    ...
	    case LISTAR_TRABAJADORES_OK:
	    	for (TTrabajador tt : (ArrayList<TTrabajador>)context.getDatos()) {
	    		String id = "" + tt.getId(), tlf = "" + tt.getTlf(), nif = tt.getNIF(), nombre = tt.getNombre(), 
    					direccion = tt.getDireccion(), idAlmacen = "" + tt.getIdAlmacen(), sueldoBase = "", horas = "", precioHora = "";
    			if (tt instanceof TTrabajadorJCompleta) {
    				sueldoBase += ((TTrabajadorJCompleta)tt).getSueldoBase();
    			}
    			else if (tt instanceof TTrabajadorJParcial){
    				horas += ((TTrabajadorJParcial)tt).getHoras();
    				precioHora += ((TTrabajadorJParcial)tt).getPrecioHora();
    			}
    			if (tt.isActivo()) {
    				id = this.toBold(id);
    				tlf = this.toBold(tlf);
    				...
    				idAlmacen = this.toBold(idAlmacen);
    				sueldoBase = this.toBold(sueldoBase);
    				horas = this.toBold(horas);
    				precioHora = this.toBold(precioHora);
    				DefaultTableCellRenderer renderer = new DefaultTableCellRenderer();
    				renderer.setFont(renderer.getFont().deriveFont(Font.BOLD));
    				table.getColumnModel().getColumn(1).setCellRenderer(renderer);
    			}
    			this.model.addRow(new Object[] {id, tlf, nif, nombre, direccion, idAlmacen, sueldoBase, horas, precioHora});
	    	}
	        break;
