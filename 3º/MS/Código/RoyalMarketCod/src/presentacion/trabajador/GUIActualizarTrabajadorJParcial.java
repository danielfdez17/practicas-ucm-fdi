...
public class GUIActualizarTrabajadorJParcial extends JFrame implements GUI {
	
	private static final String TRABAJADOR = "trabajador";
	private static final String FROM_WHERE = "GUIActualizarTrabajadorJParcial";
	private static final int BUTTONS = 8;
	private static GUIActualizarTrabajadorJParcial instancia;
	...
	public GUIActualizarTrabajadorJParcial() {
		this.guiComprobar = new GUIComprobar(TRABAJADOR);
	}
	
	public synchronized static GUIActualizarTrabajadorJParcial getInstancia() {
		if (instancia == null) instancia = new GUIActualizarTrabajadorJParcial();
		return instancia;
	}
	
	private void initGUI() {
		this.setTitle("Actualizar trabajador a jornada parcial");
		...
		this.textId.setText("" + this.ttjp.getId());
		...
		this.textDireccion.setText(this.ttjp.getDireccion());
		...
		this.textIdAlmacen.setText("" + this.ttjp.getIdAlmacen());
		panelActualizar.add(panelTarjeta.getJPanel());
		
		Panel panelHoras = new Panel("Horas");
		this.textHoras = panelHoras.getJTextField();
		this.textHoras.setText("" + this.ttjp.getHoras());
		panelActualizar.add(panelHoras.getJPanel());
		
		Panel panelPrecioHora = new Panel("Precio hora");
		this.textPrecioHora = panelPrecioHora.getJTextField();
		this.textPrecioHora.setText("" + this.ttjp.getPrecioHora());
		panelActualizar.add(panelPrecioHora.getJPanel());
		...
		buttonActualizar.addActionListener(e -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					boolean activo = this.ttjp.isActivo();
					this.clear();
					this.setVisible(false);
					Controlador.getInstancia().accion(new Context(Eventos.ACTUALIZAR_TRABAJADOR_J_PARCIAL, new TTrabajadorJParcial(id, tlf, nif, nombre, direccion, idAlmacen, horas, precioHora, activo)));
	...
	public void setttjp(TTrabajadorJParcial ttjp) {
		this.ttjp = ttjp;
	}
