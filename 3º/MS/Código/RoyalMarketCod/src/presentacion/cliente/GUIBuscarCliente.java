...
public class GUIBuscarCliente extends JFrame implements GUI {
	private static final String FROM_WHERE = "GUIBuscarCliente";
	private JTextField textId;
	private static GUIBuscarCliente instancia;
	
	public GUIBuscarCliente() {
		this.initGUI();
	}

	public synchronized static GUIBuscarCliente getInstancia() {
		if (instancia == null) instancia = new GUIBuscarCliente();
		return instancia;
	}
		...
		buttonBaja.addActionListener((e) -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					setVisible(false);
					this.clear();
					Controlador.getInstancia().accion(new Context(Eventos.BUSCAR_CLIENTE, id));
	...
	public void update(Context context) {
		...
		case BUSCAR_CLIENTE_OK:
			TCliente cliente = (TCliente) context.getDatos();
			GUIMSG.showMessage("Cliente encontrado\n" + cliente.toString(), FROM_WHERE, false);
			break;
		case BUSCAR_CLIENTE_KO:
			GUIMSG.showMessage(String.format("El cliente con id %d no existe", (int) context.getDatos()), FROM_WHERE, true);
			break;
		default:
