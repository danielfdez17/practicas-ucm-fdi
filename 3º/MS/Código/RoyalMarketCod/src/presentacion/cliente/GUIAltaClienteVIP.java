...
public class GUIAltaClienteVIP extends JFrame implements GUI {
	...
	private static final String FROM_WHERE = "GUIAltaClienteVIP";
	private static final int FIELDS = 5;
	private static GUIAltaClienteVIP instancia;
	
	private JTextField textTlf;
	private JTextField textNif;
	private JTextField textNombre;
	private JTextField textDireccion;
	private JTextField textTarjetaVIP;
	
	public GUIAltaClienteVIP() {
		this.initGUI();
	}
	
	public synchronized static GUIAltaClienteVIP getInstancia() {
		if (instancia == null) instancia = new GUIAltaClienteVIP();
		return instancia;
	}
	
	private void initGUI() {
		this.setTitle("Alta de cliente VIP");
		...
		Panel panelTarjeta = new Panel("ID tarjeta VIP");
		this.textTarjetaVIP = panelTarjeta.getJTextField();
		...
		buttonAlta.addActionListener(l -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					int idTarjetaVIP = Integer.parseInt(this.textTarjetaVIP.getText());
					this.setVisible(false);
					this.clear();
					Controlador.getInstancia().accion(new Context(Eventos.ALTA_CLIENTE_VIP, new TClienteVIP(idTarjetaVIP, tlf, nif, nombre, direccion, true)));
	...
	public void update(Context context) {
		...
		case ALTA_CLIENTE_VIP_OK:
			TClienteVIP cliente = (TClienteVIP) context.getDatos();
	...
	public String getErrorMsg(int error) {
		...
		case SACliente.CAMBIO_TIPO:
			return "El cliente existe y es un cliente normal";
		default:
	...
	public boolean areTextFieldsEmpty() {
		return this.textNombre.getText().isEmpty() 
				...
				&& this.textTarjetaVIP.getText().isEmpty();
	}
}
