...
public class GUIListarProductosPorProveedor extends JFrame implements GUI {
	
	private static final long serialVersionUID = 1L;
	
	private static final String FROM_WHERE = "GUIListarProductosPorProveedor";
	...
	private static GUIListarProductosPorProveedor instancia;
	private JTextField textId;
	
	public GUIListarProductosPorProveedor() {
		this.initGUI();
	}
	
	public synchronized static GUIListarProductosPorProveedor getInstancia() {
		if (instancia == null) instancia = new GUIListarProductosPorProveedor();
		return instancia;
	}
    	...
    	button_listar.addActionListener((e) -> {
    		if (!this.areTextFieldsEmpty()) {
    			try {
    				...
    				Controlador.getInstancia().accion(new Context(Eventos.LISTAR_PRODUCTOS_POR_PROVEEDOR, id));
    			} catch (NumberFormatException nfe) {
    				GUIMSG.showMessage("El campo 'ID proveedor' solo requiere de numeros", FROM_WHERE, true);
    			}
    		}
    		else {
