 ...
 */
public class GUIActualizarETecnico extends JFrame implements GUI{

	private static final long serialVersionUID = 1L;
	private static GUIActualizarETecnico instancia;
	private int fila = 6;
	private TEmpleadoTecnico p;

	private static final String FROM_WHERE = "GUIActualizarETecnico";
	private GUIComprobar guiComprobar;


	public GUIActualizarETecnico(){
	...
	public void setEmpleado(TEmpleadoTecnico p){
		this.p = p;
	}
	private void initGUI() {
		setLayout(null);
		this.setTitle("Actualizar Empleado Tecnico");
		...
		JLabel lrep = new JLabel("Reporte de Trabajo: ");
		...
		button.addActionListener((e)->{
			...
			if(tNombre.getText().isEmpty() || tNIF.getText().isEmpty() || tTlf.getText().isEmpty() || tDir.getText().isEmpty() || tS.getText().isEmpty() || tOfi.getText().isEmpty()){
				...
				GUIMSG.showMessage("Faltan campos por rellenar", "ACTUALIZAR EMPLEADO TECNICO", true);
				...
				try{ 
					...
					TEmpleadoTecnico emp = new TEmpleadoTecnico(tlf, nif, nombre, dir, rep, sueldo, oficina);
					
					Controlador.getInstancia().accion(new Context(Eventos.ACTUALIZAR_EMPLEADO_TECNICO, emp));
					...
					GUIMSG.showMessage("Campos erroneos", "ACTUALIZAR EMPLEADO TECNICO", true);
				}
	...
	public static GUIActualizarETecnico getInstancia() {
		if (instancia == null) instancia = new GUIActualizarETecnico();
		return instancia;
	}
