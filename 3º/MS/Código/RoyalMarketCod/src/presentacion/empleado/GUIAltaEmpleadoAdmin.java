...
public class GUIAltaEmpleadoAdmin extends JFrame implements GUI {
	
	private static final String FROM_WHERE = "GUIAltaEmpleadoAdmin";
	private static GUIAltaEmpleadoAdmin instancia;
	private static final int BUTTONS = 7;
	
	private JTextField textTlf, textNif, textNombre, textDireccion, textReporteSemanal, textSueldo, textIdOficina;
	
	public GUIAltaEmpleadoAdmin() {
		this.initGUI();
	}
	
	public synchronized static GUIAltaEmpleadoAdmin getInstancia() {
		if (instancia == null) instancia = new GUIAltaEmpleadoAdmin();
		return instancia;
	}
	
	private void initGUI() {
		setLayout(null);
		this.setTitle("Alta Empleado Administrador");
		...
		button.addActionListener((e)->{
			...
			if(tNombre.getText().isEmpty() || tNIF.getText().isEmpty() || tTlf.getText().isEmpty() || tDir.getText().isEmpty() || tS.getText().isEmpty() || tOfi.getText().isEmpty()){
				...
				GUIMSG.showMessage("Faltan campos por rellenar", "ALTA EMPLEADO ADMINISTRADOR", true);
				...
				try{ 
					...
					TEmpleadoAdministrador emp = new TEmpleadoAdministrador(tlf, nif, nombre, dir, "", sueldo, oficina);
					
					Controlador.getInstancia().accion(new Context(Eventos.ALTA_EMPLEADO_ADMINISTRADOR, emp));
					...
					GUIMSG.showMessage("Campos erroneos", "ALTA EMPLEADO ADMINISTRADOR", true);
				}
	...
	public void update(Context context) {
		...
		case ALTA_EMPLEADO_ADMINISTRADOR_OK:
			TEmpleadoAdministrador admin = (TEmpleadoAdministrador) context.getDatos();
			GUIMSG.showMessage("Se ha dado de alta al empleado administrador\n" + admin.toString(), FROM_WHERE, false);
			break;
	...
	public void clear() {
		...
		this.textReporteSemanal.setText(t);
	...
	public boolean areTextFieldsEmpty() {
		return this.textTlf.getText().isEmpty() &&
				...
				this.textReporteSemanal.getText().isEmpty() &&
