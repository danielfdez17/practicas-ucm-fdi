...
public class GUIDesvincularProyecto extends JFrame implements GUI {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final String FROM_WHERE = "GUIDesvincularProyecto";
	private static GUIDesvincularProyecto instancia;
	private JTextField textIdEmpleado, textIdProyecto;
	
	
	public GUIDesvincularProyecto() {
		this.initGUI();
	}
	
	public static GUIDesvincularProyecto getInstancia() {
		if (instancia == null) instancia = new GUIDesvincularProyecto();
		return instancia;
	}
	...
	private void initGUI() {
		...
		JButton buttonBaja = new JButton("Desvincular Proyecto");
		buttonBaja.addActionListener((e) -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					Controlador.getInstancia().accion(new Context(Eventos.DESVINCULAR_PROYECTO, transfer));
	...
	public void update(Context context) {
		...
		case DESVINCULAR_PROYECTO_OK:
			GUIMSG.showMessage("Se ha desvinculado al empleado con el proyecto", FROM_WHERE, false);
			break;
