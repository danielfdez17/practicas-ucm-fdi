...
public class GUIListarEmpleadosPorProyecto extends JFrame implements GUI {
	
	private static final String FROM_WHERE = "GUIListarEmpleadosPorProyecto";
	private static final String HEADERS[] = {"ID", "Telefono", "NIF", "Nombre",
			"Direccion", "Reporte trabajo", "Reporte semanal", "Sueldo", "ID oficina"};
	
	private static GUIListarEmpleadosPorProyecto instancia;
	private JTable table;
	private DefaultTableModel model;
	private JTextField textId;
	
	public GUIListarEmpleadosPorProyecto() {
		this.initGUI();
	}
	
	public synchronized static GUIListarEmpleadosPorProyecto getInstancia() {
		if (instancia == null) instancia = new GUIListarEmpleadosPorProyecto();
		return instancia;
	}
	
	private void initGUI() {
		this.setTitle("Listar empleados por proyecto");
		...
		buttonListar.addActionListener((e) -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					Controlador.getInstancia().accion(new Context(Eventos.LISTAR_EMPLEADOS_POR_PROYECTO, id));
		...
		Panel panelId = new Panel("ID curso");
	...
	public void update(Context context) {
		...
		case LISTAR_EMPLEADOS_POR_PROYECTO_KO:
			this.model.setRowCount(0);
			GUIMSG.showMessage("No existen empleados asociados al curso", FROM_WHERE, true);
			break;
		default:
