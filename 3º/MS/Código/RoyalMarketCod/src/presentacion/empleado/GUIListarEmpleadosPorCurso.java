...
public class GUIListarEmpleadosPorCurso extends JFrame implements GUI {
	
	private static final String FROM_WHERE = "GUIListarEmpleadosPorCurso";
	private static final String HEADERS[] = {"ID", "Telefono", "NIF", "Nombre",
			"Direccion", "Reporte trabajo", "Reporte semanal", "Sueldo", "ID oficina"};
	
	private static GUIListarEmpleadosPorCurso instancia;
	private JTable table;
	private DefaultTableModel model;
	private JTextField textId;
	
	public GUIListarEmpleadosPorCurso() {
		this.initGUI();
	}
	
	public synchronized static GUIListarEmpleadosPorCurso getInstancia() {
		if (instancia == null) instancia = new GUIListarEmpleadosPorCurso();
		return instancia;
	}
	
	private void initGUI() {
		this.setTitle("Listar empleados por curso");
		...
		buttonListar.addActionListener((e) -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					Controlador.getInstancia().accion(new Context(Eventos.LISTAR_EMPLEADOS_POR_CURSO, id));
