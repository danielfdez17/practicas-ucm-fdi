...
public class GUIVincularProducto extends JFrame implements GUI {
	
	private static final int FIELDS = 2;
	private static final String FROM_WHERE = "GUIVincularProducto";
	private JTextField textProveedor, textProducto;
	private static GUIVincularProducto instancia;
	
	public GUIVincularProducto() {
		this.initGUI();
	}
	
	public synchronized static GUIVincularProducto getInstancia() {
		if (instancia == null) instancia = new GUIVincularProducto();
		return instancia;
	}

	public void initGUI() {
		setTitle("Vincular producto");
		...
		//DesvincularButton
		JButton desvincularButton = new JButton("Vincular");
		desvincularButton.addActionListener(e -> {
			if (!this.areTextFieldsEmpty()) {
				try {
					...
					Controlador.getInstancia().accion(new Context(Eventos.VINCULAR_PRODUCTO, new TProveedorProducto(idProveedor, idProducto)));
	...
	public void update(Context context) {
		...
		case VINCULAR_PRODUCTO_OK:
			GUIMSG.showMessage("Producto vinculado con exito", FROM_WHERE, false);
			break;
	...
	public String getErrorMsg(int error) {
		...
		case SAProveedor.YA_VINCULADOS:
			return "El producto ya estaba vincualdo";
