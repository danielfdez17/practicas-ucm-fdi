...
public class GUIListarFacturasPorProducto extends JFrame implements GUI {
	
	private static final String FROM_WHERE = "GUIListarFacturasProducto";
	private static final String headers[] = { "ID", "Fecha", "ID cliente", "Coste total"};
	private static GUIListarFacturasPorProducto instancia;
	private JTextField textIdProducto;
	private JTable table;
	private DefaultTableModel model;
	
	public GUIListarFacturasPorProducto() {
		this.initGUI();
	}
	
	public synchronized static GUIListarFacturasPorProducto getInstancia() {
		if (instancia == null) instancia = new GUIListarFacturasPorProducto();
		return instancia;
	}
    ...
    private void initGUI() {
		...
		// ID
		JPanel panelIdProducto = new JPanel(new GridLayout(1, 2));
		JLabel labelIdProducto = new JLabel("ID producto: ");
		this.textIdProducto = new JTextField(10);
		panelIdProducto.add(labelIdProducto);
		panelIdProducto.add(this.textIdProducto);
		mainPanel.add(panelIdProducto, BorderLayout.PAGE_START);
    	...
    	listButton.addActionListener((e) -> {
    		if (this.areTextFieldsEmpty()) {
    			...
    			try {
    				...
    				Controlador.getInstancia().accion(new Context(Eventos.LISTAR_FACTURAS_POR_PRODUCTO, id));
	...
	public void update(Context context) {
		...
		case LISTAR_FACTURAS_POR_PRODUCTO_KO:
			GUIMSG.showMessage("No hay facturas con el producto comprado", FROM_WHERE, true);
			break;
		default:
	...
	public boolean areTextFieldsEmpty() {
		return this.textIdProducto.getText().isEmpty();
	}
}