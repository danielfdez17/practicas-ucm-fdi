...
public:
	...
	*/
	...
	////////////////////////////////
	// MODIFICACIONES
	// Coste: en el caso peor es lineal en el numero de elementos de la lista (mover el primero a la ultima posicion o viceversa)
	void add_to(int pos, int m) {
		if (m != 0) {
			Nodo* posvieja = prim;
			int cont = 0;
			// BUSCAR EL ELEMENTO DE LA POSICION pos
			while (posvieja != nullptr && cont < pos) { posvieja = posvieja->sig; cont++; }

			if (posvieja != nullptr) { // VS Studio a veces se queja si no pongo el condicional
				int antiguo = posvieja->elem;
				posvieja->elem += m;
				if (numElems != 1) {
					Nodo* posnueva = prim;

					// BUSCAR LA NUEVA POSICION
					while (posnueva != nullptr && posnueva->elem <= posvieja->elem) {
						posnueva = posnueva->sig;
					}
					if (posnueva != posvieja) {
						// DESENLAZAR EL ELEMENTO OBJETIVO
						detach(posvieja);

						// ENLAZAR EN SU NUEVA POSICION
						attach(posvieja, posnueva);
					}
				}
			}
		}
...
private:
	...
	void attach(Nodo* n, Nodo* next) {
		...
		else if (next == nullptr) { // Despues de ult
			n->ant = ult;
			ult->sig = n;
			ult = n;
			ult->sig = next;
