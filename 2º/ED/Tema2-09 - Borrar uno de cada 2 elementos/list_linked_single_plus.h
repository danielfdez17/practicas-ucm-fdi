//#pragma once
/**
 * Implementaci�n del TAD Lista mediante listas enlazadas simples.
 * Clase original de Manuel Montenegro. Adaptada por Ignacio F�bregas
 *
 * Versi�n con clase gen�rica, nodo fantasma, cabeza y cola.
 */
...
public:
    ...
    void eliminarPares() {
        ...
        int contador = 1;
        while (i != nullptr) {
            if (contador % 2 == 0) { // Si es par, se elimina y actualizan los nodos
                ant->next = sig;
                delete i;
                i = sig;
                if (i != nullptr) {
            ...
            else { // Si no, se actualizan los punteros 'contadores'
                ant = i;
                i = sig;
                if (i != nullptr) {
                    sig = i->next;
        ...
        if (ant->next == nullptr) { // En caso de que el �ltimo elemento est� en una posici�n par, se actualiza el puntero a la cola
            tail = ant;
        }
        /*while (i != nullptr) {
            if (i->value.esPar()) {
    ...
    /** versi�n no constante de front  O(1)*/
    T& front() {
        assert(head->next != nullptr);
        return head->next->value;
    }
    ...
    /** versi�n no constante de back O(1)*/
    T& back() {
        assert(tail != nullptr);
        return tail->value;
    }
    ...
    /** versi�n no constante de at O(1)*/
    T& at(int index) {
        Node* result_node = nth_node(index);
        assert(result_node != nullptr);
        return result_node->value;
