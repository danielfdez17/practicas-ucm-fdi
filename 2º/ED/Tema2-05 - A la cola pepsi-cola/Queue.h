...
public:
	...
	///////////////////////////////////
	// MODIFICACIONES
	// El coste del algoritmo en el caso peor es lineal en la suma de los tamaños de las colas (colar a los colegas al final de this)
	void cuela(T const& pringao, Queue<T> colegas) {
		Nodo* n = prim;
		// Se busca al pringao
		while (n != nullptr && n->elem != pringao) n = n->sig;
		// Si se ha encontrado al pringao
		if (n != nullptr) {
			Nodo* i = colegas.prim, * anti = nullptr;
			while (i != nullptr) {
				n->sig = i;
				colegas.prim = colegas.prim->sig;
				n = n->sig;
				anti = i;
				i = i->sig;
			}
