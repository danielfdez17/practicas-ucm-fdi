...
public:
	...
	// MODIFICACIONES
	// Complejidad: En el caso peor (el elemento b no está en la cola) el coste del algorimto es lineal en el número de elementos de la cola
	void cuela(const T&a, const T&b) {
		// BUSCAR LA PRIMERA APARICION DE a
		Nodo* na = prim;
		while (na != nullptr && na->elem != a) { na = na->sig; }
		// Si se ha encontrado el elemento a
		if (na != nullptr) {
			// BUSCAR LA PRIMERA APARICION DE b TRAS LA DE a
			Nodo* nb = na->sig, *nbant = na;
			while (nb != nullptr && nb->elem != b) { 
				nbant = nb;
				nb = nb->sig;
			}
			// Si se ha encontrado el elemento b
			if (nb != nullptr) {
				// Desenlazar b
				if (nb == ult) {
					ult = nbant;
