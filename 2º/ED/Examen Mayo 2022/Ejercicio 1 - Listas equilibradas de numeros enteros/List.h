...
public:
	...
	*/
	...
	////////////////////////////////////////
	// MODIFICACIONES
	// Sea N = lista.numElems + el numero de elementos que tenga la pila al terminar de recorrer la lista
	// El coste en el caso peor (lista llena de positivos), el coste es lineal en N (o cuadratico en lista.numElems, que es lo mismo) porque hay que anyadir al final de la lista
	// el negativo de los elementos de la pila
	void reconstruir(List<int>& lista) {
		Nodo* it = lista.prim, *sigit = it->sig;
		stack<int>s;
		while (it != nullptr) {
			// Positivo
			if (it->elem > 0) { // Se anyade a la pila
				s.push(it->elem);
			}
			// Cero
			else if (!s.empty() && it->elem == 0) { // Se actualiza el valor de it->elem con el negativo de la cima de la pila
				it->elem = -(s.top());
				s.pop();
			} // Negativo
			else {
				if (!s.empty() && s.top() == -it->elem) { // Coincide con el negativo de la cima de la pila
					s.pop();
				}
				else { // No coincide, se desenlaza y elimina
					if (it == lista.prim) { // Primero
						lista.prim = it->sig;
						if (lista.prim != nullptr)
							lista.prim->ant = nullptr;
					}
					else if (it == lista.ult) { // Ultimo
						lista.ult = it->ant;
						if (lista.ult != nullptr)
							lista.ult->sig = nullptr;
					}
					else { // Caso general
						it->sig->ant = it->ant;
						it->ant->sig = it->sig;
			...
			it = sigit;
			if (sigit != nullptr) sigit = sigit->sig;
		}
		// Se vacia la pila anyadiendo los elementos con el signo cambiado al final de la lista
		while (!s.empty()) {
			Nodo* nuevo = new Nodo(lista.ult, -s.top(), nullptr);
			if (lista.ult != nullptr)
				lista.ult->sig = nuevo;
			s.pop();
			lista.numElems++;
