...
public:
	...
	*/
	...
	void interseccion(List<int>const& aux) {
		Nodo* i = prim, * sigi = i->sig, * j = aux.prim;
		while (i != nullptr && j != nullptr) {
			if (i->elem < j->elem) { // Solo se borra si el elemento de i es menor que el de j, ya que ambas estan ordenadas
				if (i == prim) {
					prim = prim->sig;
					if (prim != nullptr)
						prim->ant = nullptr;
			...
			i = sigi;
			if (sigi != nullptr) sigi = sigi->sig;
			if (i != nullptr)
				if (j->elem < i->elem) j = j->sig;
		}
	}
	/////////////////////////////////////////////////////////////////////////////
