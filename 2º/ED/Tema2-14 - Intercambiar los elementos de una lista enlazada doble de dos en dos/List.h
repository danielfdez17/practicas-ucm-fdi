...
public:
	...
	*/
	...
	// MODIFICACIONES
	void intercambiarDeDosEnDos() {
		if (numElems < 2) return;
		Nodo* i = prim, * sigi = i->sig->sig;
		while (i != nullptr) {
			Nodo* next = i->sig;
			if (next != nullptr) {
				// Desenlazar el segundo
				if (next == ult) {
					ult = i;
					i->sig = nullptr;
				}
				else {
					next->ant->sig = next->sig;
					next->sig->ant = next->ant;
				}

				// Enlzar delante del primero
				if (i == prim) {
					prim = next;
					next->sig = i;
					i->ant = next;
			...
			i = sigi;
			if (sigi != nullptr && sigi->sig != nullptr) sigi = sigi->sig->sig;
			else i = nullptr;
		}
	}
