...
public:
	...
	//////////////////////////////////////
	// MODIFICACIONES
	// El coste del algoritmo es lineal en el numero de elementos de la cola
	void reverse() {
		if (!empty()) {
			// antn = nuevo sign; sign = antiguo antn
			Nodo* antn = nullptr, * n = prim, * sign = nullptr, * primaux = prim, * ultaux = ult;
			while (n != nullptr) {
				sign = n->sig;
				n->sig = antn;
				antn = n;
