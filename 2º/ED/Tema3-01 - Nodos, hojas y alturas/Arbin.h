...
protected:
	...
	/////////////////////////////////////////////////////////////////
	// MODIFICACIONES
	// El coste de las funciones es lineal en el numero de elemntos porque hay que recorrer el arbol entero (el del ejercicio es 3*n === n porque hay que recorrerlo 3 veces)
	static unsigned int nodosAux(Nodo* raiz) {
		if (raiz == nullptr) return 0;
		return 1 + nodosAux(raiz->iz) + nodosAux(raiz->dr);
	}
	static unsigned int alturaAux(Nodo* raiz) {
		if (raiz == nullptr) return 0;
		int i = alturaAux(raiz->iz);
		int d = alturaAux(raiz->dr);
		if (i > d) return 1 + i;
		return 1 + d;
	}
	static unsigned int hojasAux(Nodo* raiz) {
		if (raiz == nullptr) return 0;
		if (raiz->iz == nullptr && raiz->dr == nullptr) return 1;
		return hojasAux(raiz->iz) + hojasAux(raiz->dr);
	}
	/////////////////////////////////////////////////////////////////
